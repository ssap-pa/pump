...
export default function Desk({draftOwner}:{draftOwner:string|null}) {
  ...
  async function refresh() {
    try { setRows(await call<Rec[]>('/api/records')); setError(''); return true; }
    catch (e) { setError((e as Error).message); return false; }
    finally { setReady(true); }
  }
  useEffect(() => { let cancelled=false; call<Rec[]>('/api/records').then(data=>{if(!cancelled){setRows(data);setError('');}}).catch((e:Error)=>{if(!cancelled)setError(e.message);}).finally(()=>{if(!cancelled)setReady(true);}); return ()=>{cancelled=true;}; }, []);
  ...
  async function task(fn: () => Promise<void>) {
    ...
    setBusy(true);
    try { await fn(); await refresh(); toast.success('저장했어요.'); return true; }
  ...
  return <>
    ...
    <main className="shell">
      ...
      {!active ? <>
        ...
        <div className="upload-shortcuts"><button className="upload-shortcut" disabled={disabled} onClick={()=>setUploadKind('photo')}><span className="icon-tile"><Camera size={23}/></span><span><strong>현장 사진 올리기</strong><small>사진 선택 · 휴대폰 촬영</small></span><ArrowRight size={19}/></button><button className="upload-shortcut" disabled={disabled} onClick={()=>setUploadKind('quote')}><span className="icon-tile"><FileText size={23}/></span><span><strong>견적서 올리기</strong><small>PDF · 이미지 · 엑셀</small></span><ArrowRight size={19}/></button></div>
        <button className="receipt-shortcut" disabled={disabled} onClick={()=>setReceiptOpen(true)}><span className="icon-tile"><ReceiptText size={25}/></span><span><strong>영수증으로 자재 구매 내역 남기기</strong><small>사진을 읽어 품목과 금액을 자동으로 저장해요</small></span><ArrowRight size={21}/></button>
        ...
        <Tabs value={tab} onValueChange={v => setTab(String(v))}>
          ...
          <TabsContent value="quote"><div className="stack"><SiteFiles siteId={site.id} category="quote" rows={rows} onRefresh={refresh}/><Quote draftOwner={draftOwner} onBlockNavigation={setEditorBlocked} key={site.id} site={site} existing={quote(site.id)} sources={rows.filter(r=>r.kind==='quote'&&r.parent!==site.id)} profile={profile?.data || {}} busy={busy} onSave={(data) => task(async () => { await save('quote', data, site.id, quote(site.id)?.id); })} /></div></TabsContent>
          <TabsContent value="notes"><div className="stack"><SiteFiles siteId={site.id} category="photo" rows={rows} onRefresh={refresh}/><Notes draftOwner={draftOwner} onBlockNavigation={setEditorBlocked} key={site.id} site={site} rows={rows} busy={busy} onSave={(data) => task(async () => { await save('note', data, site.id); })} onRefresh={refresh} /></div></TabsContent>
          <TabsContent value="purchases"><Receipts key={site.id} siteId={site.id} rows={rows} onRefresh={refresh}/></TabsContent><TabsContent value="payments"><div className="panel stack"><div className="row"><div><p className="eyebrow">이 현장의 정산</p><h2>입금 내역</h2></div><button className="btn primary" disabled={busy} onClick={() => openPayment()}><Plus size={17} />입금 등록</button></div><div className="payment-summary"><span>아직 받을 금액</span><strong>{money(balance(site.id))}<small>원</small></strong></div>{paid(site.id) > total(quote(site.id)?.data) && <p className="notice">입금액이 견적 합계보다 많아요. 견적과 입금 내역을 확인해 주세요.</p>}{!payments.length ? <div className="empty"><span className="empty-icon"><Wallet size={28} /></span><h3>첫 입금 내역을 남겨보세요</h3><p>계약금, 중도금, 잔금을 기록하면 남은 금액이 자동으로 계산돼요.</p></div> : payments.map(r => <div className="payment-row" key={r.id}><span className="payment-check"><CircleCheck size={20} /></span><div><strong>{r.data.label}</strong><p className="muted">{r.data.date}{r.data.memo ? ' · ' + r.data.memo : ''}</p></div><strong className="payment-amount">{money(r.data.amount)}원</strong><button className="linkbutton" disabled={busy} onClick={() => openPayment(r)}>수정</button></div>)}</div></TabsContent>
        </Tabs>
